...
parser.add_argument('--soil_test', required=True, choices=['DI', 'H3A', 'M3', 'Ols', 'Both'], help='Soil test to use.')
...
if args.phosphorus_treatment == True:
    print('Yes')
    base_cols = [ 'TOC_0_2', 'IC_0_2', 'TN_0_2', 'TOC_2_6', 'IC_2_6', 'TN_2_6', 'Zone_1', 'Zone_2', 'Zone_3', 'Zone_4','PhosphorusTreatment'] #'elevation', 'slope', 'aspect', 'TC_0_2', 'TC_2_6', 
    #base_cols = ['PhosphorusTreatment']
else: 
    print('No')
    base_cols = ['TOC_0_2', 'IC_0_2', 'TN_0_2', 'TOC_2_6', 'IC_2_6', 'TN_2_6', 'Zone_1', 'Zone_2', 'Zone_3', 'Zone_4']#'elevation', 'slope', 'aspect', 'TC_0_2', 'TC_2_6', 
    #base_cols = []
...
if args.soil_test == 'Both':
    feature_cols = [col for col in temp_data.columns if col.startswith('DI')] + [col for col in temp_data.columns if col.startswith('H3A')] + base_cols
else:
    feature_cols = [col for col in temp_data.columns if col.startswith(args.soil_test)] + base_cols
...
if args.phosphorus_treatment == True:
    model_path = f"../rf_model_output_controls/models/{args.field}_{args.year}_{args.soil_test}_p.joblib"
else:
    model_path = f"../rf_model_output_controls/models/{args.field}_{args.year}_{args.soil_test}_np.joblib"
...
if args.phosphorus_treatment == True:
    results_path = f"../rf_model_output_controls/results/{args.field}_{args.year}_{args.soil_test}_p.txt"
    x_train_path = f"../rf_model_output_controls/X/{args.field}_{args.year}_{args.soil_test}_p.csv"
else:
    results_path = f"../rf_model_output_controls/results/{args.field}_{args.year}_{args.soil_test}_np.txt"
    x_train_path = f"../rf_model_output_controls/X/{args.field}_{args.year}_{args.soil_test}_np.csv"
